    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (carrierFormat === 'multi-line') {
        ...
        if (currentCarrier === 'Humana') {
          // Humana policies end with _MA, K_HMO, K_PPO, etc.
          const humanaMatch = afterCarrier.match(/^([A-Z0-9]+_[A-Z]+)(.+)$/);
...
parseTHEStatementPDF(filePath, 'test.pdf').then(records => {
  ...
  if (uhc) console.log(`  UHC: ${uhc.client} (policy: ${uhc.policyNumber})`);
  if (humana) console.log(`  Humana: ${humana.client} (policy: ${humana.policyNumber})`);
  if (aetna) console.log(`  Aetna: ${aetna.client} (policy: ${aetna.policyNumber})`);
});
