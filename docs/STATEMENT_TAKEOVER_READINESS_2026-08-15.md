...
| **Integrity** | **Needs work** | Was blocked (NHP→wrong field); P0 mapping fix shipping — re-upload or legacy fallback covers statements |
| **Marco** | **Needs work** | NHP now peels $10 going forward; remittance-only months still won't show Marco peel |

### Top 3 engineering fixes (by statement correctness impact)

1. **NHP Integrity field mapping** — ✅ Fixed on `cursor/statement-math-p0-e242`: Chris/Horacio cuts → `producer_payable`; classifier falls back for legacy `sub_agent_override` rows; CAM gets 50/25/25; Marco $10 applied on NHP.
2. **Unify override split application** — Partially done for NHP via `nhpOverrideSplit.js` → `splitFullOverridePot`. Remittance half-model still never reconstructs Marco peel.
3. **Lina holds + Med Supp / needsReview peel** — Statement Excel omits Held balances; rate engine has no Med Supp / United of Omaha table.
