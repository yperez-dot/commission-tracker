...
export default function BookOfBusiness({ user }) {
  ...
  async function runRenewalCheck() {
    ...
    setLoading(true);
    try {
      const result = await apiFetch('/bob/check-renewals', {
        method: 'POST',
        body: JSON.stringify({ period: checkPeriod })
      });
  ...
  const missingLabel = `Missing renewals (${summary?.missingCount || 0})`;
  const allLabel = `All active (${summary?.totalActive || 0})`;
      ...
      <div className="page-body">
        ...
        {summary?.missingCount > 0 && (
          <div style={{background:'var(--red-light)', border:'1px solid #F7C1C1', borderRadius:'var(--radius)', padding:'10px 14px', display:'flex', alignItems:'center', gap:10, marginBottom:14, fontSize:13, color:'#A32D2D'}}>
            ...
            <button onClick={() => setTab('missing')} style={{marginLeft:'auto', background:'none', border:'1px solid #F7C1C1', borderRadius:'var(--radius)', padding:'4px 10px', fontSize:12, color:'#A32D2D', cursor:'pointer'}}>
              Review now →
            </button>
          </div>
        )}
        ...
        <div className="kpi-grid" style={{marginBottom:14}}>
          <div className="kpi-card">
            ...
            <div className="kpi-value blue">{summary?.totalActive || 0}</div>
          </div>
          <div className="kpi-card">
            <div className="kpi-label">Missing this month</div>
            <div className={`kpi-value ${(summary?.missingCount || 0) > 0 ? 'red' : 'green'}`}>{summary?.missingCount || 0}</div>
          </div>
          <div className="kpi-card">
            <div className="kpi-label">At risk</div>
            <div className="kpi-value amber">{fmt(summary?.atRisk)}</div>
          </div>
          <div className="kpi-card">
            <div className="kpi-label">Carriers tracked</div>
            <div className="kpi-value">{summary?.byCarrier?.length || 0}</div>
          </div>
        </div>
        ...
        <div style={{display:'flex', gap:8, marginBottom:12, borderBottom:'1px solid var(--border)', flexWrap:'wrap'}}>
          <button style={tabStyle('missing')} onClick={() => setTab('missing')}>{missingLabel}</button>
          <button style={tabStyle('all')} onClick={() => setTab('all')}>{allLabel}</button>
        ...
        {(tab === 'missing' || tab === 'all') && (
          <div>
            <div className="filters">
              ...
              <span className="row-count">{tab === 'missing' ? missingClients.length : activeClients.length} clients</span>
            </div>

            <div className="card" style={{padding:0}}>
              {(tab === 'missing' ? missingClients : activeClients).length === 0 ? (
                ...
                <div className="table-wrap">
                  <table>
                    ...
                    <tbody>
                      {(tab === 'missing' ? missingClients : activeClients).map((c, i) => (
                        <tr key={c.id}>
                          ...
                          <td style={{fontWeight:600, color: c.months_missing > 0 ? 'var(--red)' : 'var(--green)'}}>
                            {fmt(c.last_commission_amount)}
                          </td>
                          <td>
                            {c.months_missing > 0
                          ...
                          <td>
                            <select
                              value={c.resolution || ''}
                              onChange={e => updateResolution(c.id, e.target.value)}
                ...
                <tbody>
                  {(summary?.byCarrier || []).map((c, i) => (
                    <tr key={i}>
                      ...
                      <td>{c.count}</td>
                      <td>
                        {clients.filter(x => x.carrier === c.carrier && x.months_missing > 0).length > 0
                      ...
                      <td>
                        {(summary?.bySource || []).find(s => s.source === 'bob_export')
                          ? <span className="badge badge-blue">BOB export</span>
                          : <span className="badge badge-gray">Statements only</span>}
                      </td>
          ...
          <div>
            <div className="card" style={{marginBottom:14}}>
              ...
              <p style={{fontSize:13, color:'var(--text-muted)', marginBottom:12}}>
                Select a statement period and run the check — the system compares every client in your BOB against that month's commission records and flags anyone who didn't get paid.
              </p>
            ...
            <div className="card" style={{marginBottom:14}}>
              ...
              <p style={{fontSize:13, color:'var(--text-muted)', marginBottom:12}}>
                Automatically populate your Book of Business from all commission records you've already uploaded. Every unique client + carrier combination becomes a BOB entry. Run this once to get started.
              </p>
              <button className="btn btn-primary" onClick={buildFromStatements}>
                Build BOB from statements →
              </button>
            </div>
            ...
            <div className="card">
              ...
              <p style={{fontSize:13, color:'var(--text-muted)', marginBottom:12}}>
                Upload a BOB export file directly from a carrier portal. This gives more accurate tracking than statements alone.
              </p>
              {uploadStatus && (
