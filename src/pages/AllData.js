...
export default function AllData({ user, initialFilters = {} }) {
  ...
  // If initialFilters change (new navigation), update filters
  useEffect(() => {
  ...
  function exportCSV() {
    ...
    const rows = records.map(r => [
      r.agent_name, r.carrier, r.client_full_name, r.effective_date,
      r.premium, r.commission, r.classification, r.payment_period, r.policy_number
    ]);
    ...
    <>
      ...
      <div className="page-body">
        ...
        <div className="filters" style={{marginBottom:12}}>
          ...
          {hasFilters && (
            <button className="btn" onClick={() => { setAgent(''); setCarrier(''); setPeriod(''); setClassification(''); setPage(0); }}
              style={{fontSize:11, color:'var(--red)'}}>
              Clear filters
            </button>
          )}
          <span className="row-count">{total.toLocaleString()} records</span>
          <button className="btn btn-primary" onClick={exportCSV} disabled={!records.length} style={{marginLeft:'auto'}}>
            ↓ Export CSV
          </button>
        </div>
        ...
        {hasFilters && (
          <div style={{
            ...
            padding:'8px 14px',marginBottom:12,fontSize:12,color:'#0C447C',
                      ...
                      <tr key={r.id}>
                        ...
                        <td>
                          <span className={`badge ${
                            ...
                            r.classification==='Chargeback'?'badge-red':'badge-gray'
