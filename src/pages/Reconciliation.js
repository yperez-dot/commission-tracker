...
export default function Reconciliation({ user }) {
  ...
  useEffect(() => {
    apiFetch('/records/filters').then(d => {
      ...
      if (d.periods?.length) setSelectedPeriod(d.periods[0]);
    }).catch(console.error);
  }, []);
                  ...
                  <table>
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td style={{fontWeight:600, color:'var(--green)'}}>{fmt(nhpPreSplitTotal)}</td>
              ...
              <div className="card" style={{padding:0}}>
                <div className="table-wrap">
                  <table>
                    ...
                    <tbody>
                      {nhpRecords.map((r, i) => {
                        const eligible = isSplitEligible(r.effective_date);
