...
function generateStatement(agent, records, periodLabel, total, isBSI) {
  ...
  const rows = records.map(r => [
    ...
    r.effective_date||'—', fmt(r.commission), r.classification||'—'
  ]);
  const csv = [
    [`${agencyName} — Commission Statement`],
    [`Period: ${periodLabel}`],
    [`Agent: ${agent}`],
    [`Total: ${fmt(total)}`],
    [`Generated by OliComm`],
    [],
    headers,
    ...rows
...
export default function Payroll({ user }) {
  ...
  async function loadPayouts(period) {
    ...
    try {
      ...
      if (isBSI) {
        // BSI: show all agents with positive commissions (they ARE the agents)
        allRecs = allRecs.filter(r => parseFloat(r.commission) > 0);
      } else {
      ...
      setPayouts(Object.values(grouped).filter(p => p.total > 0).sort((a,b) => b.total - a.total));
  ...
  function exportAll() {
    ...
    for (const p of payouts) {
      ...
      for (const r of p.records) lines.push(`"${r.policy_number||''}","${r.client_full_name}","${r.carrier}","${r.effective_date}","${r.commission}","${r.classification||''}"`);
      lines.push(``);
    }
