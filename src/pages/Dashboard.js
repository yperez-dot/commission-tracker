...
// ─── Bar chart (ComTrack-style vertical) ─────────────────────────────────────
function VerticalBarChart({ data, loading }) {
  if (loading) return <div style={{height:220,display:'flex',alignItems:'center',justifyContent:'center',color:'var(--text-muted)',fontSize:12}}>Loading...</div>;
  if (!data || !data.length) return <div style={{height:220,display:'flex',alignItems:'center',justifyContent:'center',color:'var(--text-muted)',fontSize:12}}>No data for selected filters</div>;

  const max = Math.max(...data.map(d => Math.abs(parseFloat(d.total) || 0)));

  return (
    <div style={{display:'flex',alignItems:'flex-end',gap:8,height:220,paddingBottom:28,paddingTop:8,position:'relative',overflowX:'auto'}}>
      {/* Y-axis labels */}
      <div style={{position:'absolute',left:0,top:0,bottom:28,display:'flex',flexDirection:'column',justifyContent:'space-between',pointerEvents:'none'}}>
        {[1,0.75,0.5,0.25,0].map(f => (
          <span key={f} style={{fontSize:9,color:'var(--text-muted)',lineHeight:1}}>{fmtK(max*f)}</span>
        ))}
      </div>
      <div style={{flex:1,display:'flex',alignItems:'flex-end',gap:6,paddingLeft:36,height:'100%'}}>
        {data.map((d, i) => {
          ...
          const isNeg = val < 0;
          return (
            <div key={i} style={{flex:1,minWidth:40,maxWidth:80,display:'flex',flexDirection:'column',alignItems:'center',height:'100%',justifyContent:'flex-end'}}>
              <span style={{fontSize:10,fontWeight:600,color:isNeg?'#E24B4A':'var(--text)',marginBottom:4,whiteSpace:'nowrap'}}>{fmtK(val)}</span>
              <div style={{
                width:'100%',
                height:`${pct}%`,
                minHeight:4,
                background:isNeg?'#E24B4A':'#185FA5',
                borderRadius:'4px 4px 0 0',
                transition:'height 0.4s ease',
                cursor:'default'
              }}/>
              <span style={{fontSize:9,color:'var(--text-muted)',marginTop:6,textAlign:'center',whiteSpace:'nowrap',overflow:'hidden',maxWidth:'100%',textOverflow:'ellipsis'}}>{label}</span>
            </div>
          );
        })}
      </div>
...
// ─── Horizontal bar for agents/carriers ──────────────────────────────────────
function HBar({ data, color, onClickItem, loading }) {
  if (loading) return <div style={{padding:20,color:'var(--text-muted)',fontSize:12,textAlign:'center'}}>Loading...</div>;
  if (!data || !data.length) return <div style={{padding:20,color:'var(--text-muted)',fontSize:12,textAlign:'center'}}>No data</div>;
  const max = Math.max(...data.map(d => Math.abs(parseFloat(d.total)||0)));
  return (
        ...
        return (
          <div key={i} onClick={()=>onClickItem&&onClickItem(d)} style={{cursor:onClickItem?'pointer':'default'}}>
            <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
              <span style={{fontSize:12,color:onClickItem?'#185FA5':'var(--text)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',maxWidth:'65%',fontWeight:500}} title={name}>{name}</span>
              <span style={{fontSize:12,fontWeight:700,color:isNeg?'#E24B4A':color}}>{fmt(val)}</span>
            </div>
  ...
  return (
    <div style={{marginBottom:18}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8}}>
        <span style={{fontSize:10,fontWeight:700,textTransform:'uppercase',letterSpacing:'0.7px',color:'var(--text-muted)'}}>{title}</span>
        <button onClick={()=>allSel?onClearAll():onSelectAll(items)} style={{fontSize:10,color:'#185FA5',background:'none',border:'none',cursor:'pointer',padding:0,fontWeight:600}}>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:1}}>
        {items.map(item => {
          ...
          return (
            <label key={item} style={{display:'flex',alignItems:'center',gap:8,padding:'4px 6px',borderRadius:5,cursor:'pointer',background:isSel?'#EBF3FC':'transparent',transition:'background 0.1s'}}>
...
export default function Dashboard({ user, onNavigate }) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // Build period trend data from byPeriod summary
  ...
  return (
    <div style={{display:'flex',height:'100vh',overflow:'hidden',background:'var(--gray-50)'}}>
      ...
      {/* ── Left filter sidebar ── */}
      <div style={{width:210,minWidth:210,background:'var(--bg)',borderRight:'1px solid var(--border)',overflowY:'auto',padding:'16px 12px',flexShrink:0,display:'flex',flexDirection:'column'}}>
        {/* Sidebar header */}
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14}}>
          ...
          {hasFilters
            ...
            : <span style={{fontSize:11,color:'var(--text-muted)'}}>{totalFiltersActive === 0 ? 'None active' : ''}</span>
          }
        </div>
        {hasFilters && (
          <div style={{background:'#EBF3FC',borderRadius:6,padding:'5px 10px',marginBottom:14,fontSize:11,color:'#185FA5',fontWeight:600}}>
        ...
        <FilterGroup title="Agent" items={agentList} selected={selAgents}
          ...
          onSelectAll={items=>setSelAgents([...items])}
          onClearAll={()=>setSelAgents([])}
        />
        ...
        <FilterGroup title="Plan Type" items={allFilters.planTypes||[]} selected={selPlanTypes}
          ...
          onSelectAll={items=>setSelPlanTypes([...items])}
          onClearAll={()=>setSelPlanTypes([])}
        />
        <div style={{borderTop:'1px solid var(--border)',marginBottom:16}}/>
        ...
        <div style={{padding:'16px 24px 0',flexShrink:0}}>
            ...
            {hasFilters && (
              <div style={{display:'flex',flexWrap:'wrap',gap:4,justifyContent:'flex-end',maxWidth:500}}>
                {[...selAgents,...selCarriers,...selPeriods.map(formatPeriod),...selTypes,...selPlanTypes].filter(Boolean).map(f=>(
                  <span key={f} style={{background:'#185FA5',color:'#fff',borderRadius:4,padding:'2px 8px',fontSize:10,fontWeight:600}}>{f}</span>
                ))}
              </div>
            )}
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:16}}>
            {[
              ...
              <div key={i} style={{background:'var(--bg)',borderRadius:10,padding:'14px 16px',border:'1px solid var(--border)',boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
                ...
                <div style={{fontSize:22,fontWeight:800,color:card.color,lineHeight:1}}>{loading ? '—' : card.value}</div>
        ...
        <div style={{padding:'0 24px 24px',flex:1,display:'flex',flexDirection:'column',gap:14}}>
          ...
          <div style={{background:'var(--bg)',borderRadius:10,border:'1px solid var(--border)',padding:'16px 20px',boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:4}}>
              <div style={{fontSize:14,fontWeight:700,color:'var(--text)'}}>Commission &amp; Count</div>
              <div style={{display:'flex',alignItems:'center',gap:14,fontSize:11,color:'var(--text-muted)'}}>
                <span style={{display:'flex',alignItems:'center',gap:4}}><span style={{width:10,height:10,borderRadius:2,background:'#185FA5',display:'inline-block'}}/> Commission</span>
          ...
          {/* Two-column: by agent + by carrier */}
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
            <div style={{background:'var(--bg)',borderRadius:10,border:'1px solid var(--border)',padding:'16px 20px',boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
              <div style={{fontSize:13,fontWeight:700,color:'var(--text)',marginBottom:14}}>By Agent</div>
              <HBar data={summary?.byAgent||[]} color="#185FA5" onClickItem={d=>drillDown({agent:d.agent_name})} loading={loading} />
            </div>
            <div style={{background:'var(--bg)',borderRadius:10,border:'1px solid var(--border)',padding:'16px 20px',boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
              <div style={{fontSize:13,fontWeight:700,color:'var(--text)',marginBottom:14}}>By Carrier</div>
          ...
          <div style={{background:'var(--bg)',borderRadius:10,border:'1px solid var(--border)',boxShadow:'0 1px 3px rgba(0,0,0,0.04)',overflow:'hidden'}}>
            <div style={{padding:'12px 16px',borderBottom:'1px solid var(--border)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
              <div>
                <span style={{fontSize:14,fontWeight:700,color:'var(--text)'}}>KPIs</span>
                <span style={{fontSize:12,color:'var(--text-muted)',marginLeft:10}}>Total Agents: {kpi?.agents?.length||0}</span>
              </div>
              {kpi?.agents?.length > 0 && (
                <button onClick={exportKPI} style={{background:'none',border:'1px solid var(--border)',borderRadius:6,padding:'5px 12px',fontSize:12,cursor:'pointer',color:'var(--text)',display:'flex',alignItems:'center',gap:4}}>
                  ↓ Download
                </button>
              )}
            </div>
                ...
                {kpi?.totals && !loading && (
                  <tfoot>
                    <tr style={{background:'#185FA5',fontWeight:700}}>
                      <td colSpan={2} style={{padding:'10px 12px',color:'#fff',fontSize:12}}>Total ({kpi.agents.length} agents)</td>
                      <td style={{padding:'10px 12px',color:'#fff',fontSize:12}}>{fmt(kpi.totals.total_commission)}</td>
                      <td style={{padding:'10px 12px',color:'#fff',fontSize:12}}>{(kpi.totals.total_count||0).toLocaleString()}</td>
                      <td style={{padding:'10px 12px',color:'rgba(255,255,255,0.7)',fontSize:12}}>100%</td>
                      <td style={{padding:'10px 12px',color:'#fff',fontSize:12}}>{fmt(kpi.totals.advance_amount)}</td>
                      <td style={{padding:'10px 12px',color:'#fff',fontSize:12}}>- {fmt(kpi.totals.chargeback_amount)}</td>
                      <td style={{padding:'10px 12px',color:'rgba(255,255,255,0.8)',fontSize:12}}>
                        {kpi.totals.total_commission>0 ? fmtPct(kpi.totals.chargeback_amount/Math.abs(kpi.totals.total_commission)*100) : '0.00%'}
                      </td>
                      <td style={{padding:'10px 12px',color:'#fff',fontSize:12}}>{fmt(kpi.totals.total_commission-(kpi.totals.chargeback_amount||0))}</td>
                      <td style={{padding:'10px 12px',color:'#fff',fontSize:12}}>{kpi.totals.new_apps}</td>
                      <td style={{padding:'10px 12px',color:'#fff',fontSize:12}}>{kpi.totals.advance_count}</td>
                      <td style={{padding:'10px 12px',color:'#fff',fontSize:12}}>{kpi.totals.chargeback_count}</td>
                    </tr>
                  </tfoot>
                )}
              </table>
